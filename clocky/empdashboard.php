...
$active_work = null;
$has_used_break_today = false;

if ($emp_id) {
    // Aktív munka lekérése
    ...
    // Napi szünet ellenőrzése (Volt-e már ma elindított szünete?)
    $check_break_today = "SELECT wt_ID FROM worktime WHERE FK_empID = ? AND DATE(start_datetime) = CURDATE() AND startbreak_time IS NOT NULL LIMIT 1";
    $stmt_break = $conn->prepare($check_break_today);
    $stmt_break->bind_param("i", $emp_id);
    $stmt_break->execute();
    if ($stmt_break->get_result()->num_rows > 0) {
        $has_used_break_today = true;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        elseif (isset($_POST['start_break']) && $active_work) {
            // Szerver oldali dupla védelem: csak akkor indíthat, ha még nem volt ma szüneten
            if (!$has_used_break_today) {
                $wt_id = (int)$active_work['wt_ID'];
...
<head>
    ...
    <style>
        ...
        .btn:hover:not(:disabled) { transform: translateY(-4px); filter: brightness(1.1); }
            ...
            <form method="POST" id="workForm">
                ...
                ?>
                    ...
                    <?php else: ?>
                        ...
                        <?php if ($has_used_break_today): ?>
                            <div class="used-break-msg">A mai napi szünetedet már felhasználtad.</div>
                            <button type="button" class="btn" disabled>
                                <i class="fas fa-ban" style="margin-right: 10px;"></i> Nincs több szünet
                            </button>
                        <?php else: ?>
