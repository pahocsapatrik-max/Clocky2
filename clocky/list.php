...
if ($role === 1) {
    ...
    $sql = "SELECT 
            ...
            JOIN emp e ON w.FK_empID = e.empID
            JOIN role r ON w.FK_roleID = r.roleID
            JOIN users u ON e.FK_userID = u.id 
            WHERE u.username = ?
            ORDER BY w.start_datetime DESC";
    $stmt = $conn->prepare($sql);
                    ...
                    <?php while($row = $result->fetch_assoc()): ?>
                        ...
                        <tr class="log-row">
                            <td data-label="Dolgozó" class="search-name">
                                <strong><?php echo htmlspecialchars($row['dolgozo_neve']); ?></strong>
                            </td>
                            <td data-label="Munkakör" class="search-role">
                                <?php echo htmlspecialchars($row['role_name']); ?>
                            </td>
                            <td data-label="Kezdés">
                                <?php echo $start->format('Y.m.d. H:i'); ?>
                            </td>
                            <td data-label="Befejezés">
                                <?php echo $row['end_datetime'] ? date('H:i', strtotime($row['end_datetime'])) : "---"; ?>
                            </td>
...
<script>
    ...
    document.getElementById('logSearch').addEventListener('keyup', function() {
        ...
        // Ha nincs találat, mutassuk az üzenetet (opcionális)
        const noData = document.getElementById('noDataRow');
        if(noData) noData.style.display = hasVisibleRow ? "none" : "";
    });
</script>
