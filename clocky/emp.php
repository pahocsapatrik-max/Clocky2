...
// Docker kompatibilis: a dinamikus $base_url-t használja az átirányításhoz
if (strpos($request_uri, '.php') !== false && !isset($_GET['api']) && !isset($_GET['export'])) {
    $pretty_name = $routes[$current_file] ?? 'dolgozok';
    header("HTTP/1.1 301 Moved Permanently");
    header("Location: " . $base_url . $pretty_name);
    exit();
}


// --- RESTful API & ADATKEZELÉS ---
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    $filename = "dolgozok_" . date('Y-m-d') . ".csv";
    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="' . $filename . '"');
    
    // UTF-8 BOM a helyes Excel megjelenítéshez
    echo "\xEF\xBB\xBF";
    
    $output = fopen('php://output', 'w');
    // Fejléc kiírása 
    fputcsv($output, ["ID", "Nev", "Szuletes", "Munkaido", "Munkakor", "Email"], ";");
...
<div class="glass-card">
    ...
    <form method="GET" action="<?php echo $base_url; ?>dolgozok" class="search-container">
        ...
        <a href="<?php echo $base_url; ?>dolgozok?export=csv&search=<?php echo urlencode($search); ?>" class="btn btn-secondary"><i class="fas fa-file-export"></i>&nbsp;CSV Export</a>
    </form>
